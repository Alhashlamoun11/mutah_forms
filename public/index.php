...
$data=[];
$forms=$dbConnection->select("forms F",[
        'F.id as fid',
    'F.title',
    'F.role',
    'F.created_at',
    'F.status',
    '    CASE WHEN A.id IS NOT NULL THEN A.id ELSE NULL END AS aid',
    'CASE WHEN A.created_at IS NOT NULL THEN A.created_at ELSE NULL END AS answered_date'
    ],[],['left join answer_checker A on (A.form_id=F.id AND A.user_id='.$_SESSION['id'].')']);
if($forms){
    foreach ($forms as $form){
        if(auth_form_question($form['fid'],$form['role'],'forms')) {
//            echo " ".$form['role']." ".$form['id']."<pre>";
            $data[]=$form;
        }
...
<head>
    ...
    <link rel="icon" href="../assets/images/background/111mutah.jpg" type="image/x-icon">
    <style>
...
<div class="container" style="direction:rtl">
    <a style="float:right;" class="btn btn-success" href="/logout">خروج</a>
    ...
    <div class="card">
        <div class="table-responsive">
            <table class="table table-striped table-bordered no-footer responsive" style="font-size:13px">
                <thead>
                <tr><th>#</th>
                    <th>الاسم</th>
                    <th>حالة النموذج</th>
                ...
                <?php foreach($data as $datum){?>
                    <tr>
                        <td>
                            <a <?php echo $datum['status']?"href=/public/form?id=".$datum['fid']:""?>>
                                <?php echo $datum['fid']?>
                            </a>
                        </td>
                        ...
                        <?php if($datum['status']){?>
                            <td style="min-width:80px;color:green">نشط</td>
                        <?php } else{?>
                            <td style="min-width:80px;color:#800000">غير نشط</td>
                        <?php }?>

                        <?php if($datum['aid']){?>
                            <td style="min-width:80px;color:green">تم التقدم</td>
                        <?php } else{?>
                            <td style="min-width:80px;color:#800000">لم يتم التقدم</td>
                        <?php }?>

                        <td style="font-size:12px;min-width:120px"><?php echo $datum['answered_date']??"-------------------------"?></td>
                        <td style="min-width:120px"><?php echo $datum['created_at']?></td>
